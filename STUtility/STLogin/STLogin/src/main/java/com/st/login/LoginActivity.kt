...
import com.st.login.loginprovider.PredMntLoginProvider
...
class LoginActivity : AppCompatActivity() {
    ...
    private fun initLoginProvider(loginProviderType: String){
        ...
        if(LoginProviderFactory.LoginProviderType.valueOf(loginProviderType) == LoginProviderFactory.LoginProviderType.KEYCLOAK){
            setupLoginPage(provider, R.drawable.logo_keycloak, R.string.sign_in_with_keycloak, R.drawable.keycloak_logo_login)
            configuration = Configuration.getInstance(applicationContext, com.st.login.R.raw.auth_config_keycloak)
        }else if (LoginProviderFactory.LoginProviderType.valueOf(loginProviderType) == LoginProviderFactory.LoginProviderType.PREDMNT) {
            setupLoginPage(provider, R.drawable.predictive_maintenance_light_blue, R.string.sign_in_with_myst, R.drawable.myst_logo_login)
            configuration = Configuration.getInstance(applicationContext, com.st.login.R.raw.auth_config_predictive)
        }
        else {
            setupLoginPage(provider, R.drawable.logo_atr, R.string.sign_in_with_myst, R.drawable.myst_logo_login)
            configuration = Configuration.getInstance(applicationContext, com.st.login.R.raw.auth_config_cognito)
        }
    ...
    private fun setupLoginPage(provider: LoginProviderFactory.LoginProviderType, imageType: Int, textType: Int, logoProvider: Int) {
                    ...
                    if (loginProvider.getLoginProviderTag() == "PREDMNT_LOGIN_PROVIDER") {
                        ...
                        webView.webViewClient = object : WebViewClient() {
                            ...
                            override fun shouldOverrideUrlLoading(view: WebView, url: String): Boolean {
                                ...
                                if (url.startsWith("INSERT_PARAMETER")) {
    ...
    private fun setupWarningLayout(){
        ...
        tvError.text = "For the first time, please make authentication at this link: https://dsh-predmnt.st.com/Signin"
