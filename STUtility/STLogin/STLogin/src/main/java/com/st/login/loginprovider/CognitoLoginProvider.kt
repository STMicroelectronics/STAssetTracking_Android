...
open class CognitoLoginProvider(
    ...
    override suspend fun getAuthData(): AuthData? {
        ...
        runBlocking {
            ...
            suspendCoroutine<Unit> { continuation ->
                ...
                val jwt = JWT()
                Log.d("AUTH N NEEDS_REFRESH?", mAuthStateManager.current.needsTokenRefresh.toString())
                ...
                mAuthStateManager.current.performActionWithFreshTokens(mAuthService) { accessToken, idToken, ex ->
                    if(ex != null){
                        ...
                        Log.d("AUTH N EXPIRATION", authN!!.expiration)
                        ...
                        if (authZ != null) {
                            ...
                            if (authZ.expiration!! > timestampToday) {
                                ...
                                val refreshedAuthZToken = refreshAuthZToken(authN!!.token, authN!!.accessKey) //AuthZ expired - refresh it
    ...
    suspend fun fetchAuthzToken(idToken: String?, accessToken: String?): LoginResult{
        ...
        try {
            ...
            withContext(Dispatchers.IO) {
                val authzTokenEndpoint = getAuthZTokenURL()
                response = getAuthZRequest(authzTokenEndpoint, idToken, accessToken).inputStream.source().buffer()
                    .readString(Charset.forName("UTF-8"))
                response = extractAuthZToken(response)
            }
    ...
    protected open fun getAuthZRequest(authzTokenEndpoint: String, idToken: String?, accessToken: String?): HttpURLConnection{
