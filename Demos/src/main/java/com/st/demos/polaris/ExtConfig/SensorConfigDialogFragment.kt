...
class SensorConfigDialogFragment(val node: Node?) : androidx.fragment.app.DialogFragment() {
    ...
    private val mSensorsAdapter = ExtConfigurationSensorViewAdapter(
            object : ExtConfigurationSensorViewAdapter.SensorInteractionCallback {
                override fun onSensorCollapsed(selected: ExtConfigurationViewModel.SensorViewData) {
                    viewModel.collapseSensor(selected)
                }

                override fun onSensorExpanded(selected: ExtConfigurationViewModel.SensorViewData) {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        loadMLCViewModel = ViewModelProvider(requireActivity()).get(
            ExtConfigurationMLCConfigViewModel::class.java)
