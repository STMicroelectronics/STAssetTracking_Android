...
import com.st.assetTracking.atrBle1.*
...
class BleProvisioningDevice : ActivityWithNode() {

    companion object {
        fun startWithNode(context: Context, node: Node): Intent {
            return getStartIntent(context, BleProvisioningDevice::class.java, node, false)
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mViewModel.atrFirmware.observe(this, Observer { atrFw ->
            if (!atrFw) {
                ...
                this@BleProvisioningDevice.onBackPressed()
            }
        })
    ...
    private fun setBoardImageResized(ivBoard: ImageView) {
        Glide
            ...
            .load(R.drawable.real_board_sensortilebox)
            .fitCenter()
            .into(ivBoard)
    }

    private fun provisioningDevice(
        bleProvisioningDevice: BleProvisioningDevice,
        ...
        loginManager.getAtrAuthData { authData ->
            if (authData != null) {
                ProvisioningDevice.startActivityToProvisioningFrom(
                    ...
                    Device.Type.SENSORTILEBOX.toString(),
                    node.device.address,
                    null,
                    null
                )
